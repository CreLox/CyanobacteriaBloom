...
To rotate the model in the opposite direction, use ```turn y -1 360``` instead.

To show hbonds between two specific residues: ```hbonds /A:85 restrict /B:42```.

To define amino acids in a protein chain (```/B```) contacting the ligand (```FAD```): ```contacts :FAD restrict /B overlapCutoff -0.4 hbondAllowance 0.0 select true reveal true```
